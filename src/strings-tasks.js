...
function countVowels(str) {
  ...
  let sum = 0;
  str.split('').forEach((element) => {
    if (vowels.includes(element) || vowels.includes(element.toLowerCase())) {
      sum += 1;
    }
  });
  return sum;
...
function reverseWords(str) {
  ...
  const newArray = arrayWords.map((element) => {
...
function encodeToRot13(str) {
  const firstArray = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const secondArray = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
